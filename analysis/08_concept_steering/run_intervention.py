...
from lib.nulls import shuffled_tag_indices
...
def main(config_path: Path, force: bool = False) -> int:
    ...
    # 1. Hidden states for all fables at the requested layers (single pass)
    ...
    # baseline group MRRs (per-concept, derived inline)
    ...
    null_envelopes = None
    if cfg["null_controls"]["run_mode"] != "skip":
    ...
    # 6. Final summary with null envelopes + plot + decision
