"""
ft_10 Option B — Stage 2: MORABLES Fine-tuning from IdioLink Pre-trained Checkpoint.

Loads the Stage 1 merged model (pretrain.py output), applies a fresh LoRA adapter,
and runs MORABLES 5-fold CV with the same MaskedInfoNCE + FN-masking setup as ft_09.

Stage 1 must complete before this script is run:
    ./run.sh finetuning/ft_10_idiolink_transfer/option_b_sequential/pretrain.py --remote --gpu 1

Then run Stage 2:
    ./run.sh finetuning/ft_10_idiolink_transfer/option_b_sequential/finetune.py --remote --gpu 1
    ./run.sh finetuning/ft_10_idiolink_transfer/option_b_sequential/finetune.py --fold 0 --remote --gpu 1
    ./run.sh finetuning/ft_10_idiolink_transfer/option_b_sequential/finetune.py --force --remote --gpu 1
"""
import argparse
import gc
import json
...
CONFIG_PATH = EXP_DIR / "config_finetune.yaml"

SIM_MATRIX_PATH = ROOT / "data/processed/moral_sim_matrix.npy"


# ── False-negative-aware InfoNCE loss (identical to ft_09) ────────────────────
...
class MaskedInfoNCELoss(nn.Module):
    ...
    def forward(self, sentence_features: list, labels=None) -> torch.Tensor:
        ...
        fable_sim = torch.mm(moral_emb, pos_emb.T) / τ

        if labels is not None:
            batch_sim = self.moral_sim_matrix[labels][:, labels]
            false_neg = batch_sim > self.fn_threshold
            false_neg.fill_diagonal_(False)
            fable_sim = fable_sim.masked_fill(false_neg, float("-inf"))

        moral_sim = torch.mm(moral_emb, moral_emb.T) / τ
...
# ── Best adapter callback (identical to ft_09) ─────────────────────────────────
...
def build_model(config: dict):
    """Load Stage 1 merged model from disk, then apply fresh LoRA for Stage 2."""
    from sentence_transformers import SentenceTransformer
    ...
    print(f"  Loading Stage 1 model from {model_name} …")
    model = SentenceTransformer(model_name, model_kwargs=model_kwargs or None)
...
# ── Fold generation (identical to ft_09) ──────────────────────────────────────
...
def run_fold(
    ...
    model_cache    = Path(config["model_output_dir"]) / f"fold_{fold_idx}"
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            name=f"ft_10b_fold_{fold_idx}",
            group="ft_10_option_b",
            tags=["ft_10", "option_b", f"fold_{fold_idx}"],
    ...
    if model_cache.exists() and not force:
        ...
        model = SentenceTransformer(str(model_cache), model_kwargs=model_kwargs or None)
    else:
        import shutil
        ...
        if os.getenv("TG_BOT_TOKEN") and os.getenv("TG_CHAT_ID"):
            ...
            callbacks.append(TelegramCallback(label=f"ft_10b/fold_{fold_idx}"))
    ...
    with open(RESULTS_DIR / f"fold_{fold_idx}.json", "w") as f:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ft_10 Option B — Stage 2: MORABLES fine-tuning")
    ...
    # Guard: Stage 1 must exist before Stage 2 can run
    stage1_path = Path(config["model_name"])
    if not stage1_path.exists():
        print(f"[finetune] ERROR: Stage 1 model not found at {stage1_path}")
        print("[finetune] Run pretrain.py first.")
        sys.exit(1)

    print(f"\n[ft_10b finetune]  stage1={config['model_name']}  doc_mode={config['doc_mode']}")
    ...
    if args.fold is None and len(folds) > 1:
        ...
        for fold in sorted(folds, key=lambda f: f["fold"]):
            result_file = RESULTS_DIR / f"fold_{fold['fold']}.json"
    ...
    out = RESULTS_DIR / f"{ts}_option_b.json"
