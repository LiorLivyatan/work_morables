"""
ft_01_5fold_cv — standalone evaluation.

Loads cached per-fold models and evaluates each on its held-out test split
against the full 709-fable corpus. Also runs the baseline (untrained model)
on every fold for a direct side-by-side comparison.
...
Usage
-----
    python finetuning/ft_01_5fold_cv/evaluate.py
    python finetuning/ft_01_5fold_cv/evaluate.py --fold 0
    python finetuning/ft_01_5fold_cv/evaluate.py --doc_mode fable_plus_summary
    python finetuning/ft_01_5fold_cv/evaluate.py --wandb    # log results to wandb
"""
import argparse
import json
import sys
...
def print_fold_table(
    ...
    print(f"\n{'─' * 70}")
    print(f"  ft_01_5fold_cv  |  doc_mode={config['doc_mode']}  |  model={config['model_name']}")
    print(_divider())
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Standalone evaluation for ft_01_5fold_cv")
    ...
    # Verify requested folds have cached models before loading data
    missing = [
    ...
    if missing:
        raise FileNotFoundError(
            ...
            f"Run training first:  python finetuning/ft_01_5fold_cv/train.py --doc_mode {config['doc_mode']}"
        )

    print(
        f"\n[ft_01_5fold_cv / evaluate]  "
    ...
    # ── Baseline: one model instance, evaluated on every fold's test split ───
    from sentence_transformers import SentenceTransformer
    print("\n  Loading baseline (untrained model)...")
    ...
    if args.use_wandb:
        ...
        wandb.init(
            ...
            tags=["ft_01_5fold_cv", config["doc_mode"], "evaluate"],
