...
INSTRUCTIONS = {
    ...
    "summarize_match": (
        "Given a moral lesson, retrieve the fable whose core message matches this moral"
    ),
    "cot_style": (
        "Given a moral, reason about what story would teach this lesson, then retrieve it"
    ),
    "summary_retrieval": (
        "Given a moral, retrieve the fable whose summary best matches this moral"
    ),
    "baseline_x2": (
        "Given a text, retrieve the most relevant passage that matches this text"
    ),
...
# Prompt repetition (Leviathan et al., 2025): repeat the full formatted query
# so that later tokens attend to earlier ones via causal attention.
PROMPT_REPEAT = {
    "baseline_x2": 2,
    "baseline_x3": 3,
}
...
class Qwen3Encoder:
    ...
    def encode_queries(self, texts: list[str], instruction: str,
                       batch_size: int = 16, repeat: int = 1) -> np.ndarray:
        formatted = [f"Instruct: {instruction}\nQuery:{t}" for t in texts]
        if repeat > 1:
            formatted = [q * repeat for q in formatted]
