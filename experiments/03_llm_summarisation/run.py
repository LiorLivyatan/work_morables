...
# ── Approach B: generation models (Qwen3.5, generated via mlx_lm) ─────────────
# Summaries were generated by 07_generate_summaries.py using mlx_lm with Qwen3.5.
# Keys must match the folder names under results/summaries/.
QWEN_MODELS = {
...
CORPUS_TEMPLATES = {
    ...
    # TODO: add only moral - expect to get near 1.0
}
...
Given a moral lesson and a list of candidate fables, identify which fable best \
illustrates this moral.
...
Output ONLY a line starting with "RANKING:" followed by the fable numbers in order \
from best to worst match, separated by commas.
...
def _find_latest_predictions(model_run_dir: Path | None) -> Path:
    ...
    # Look for model comparison results from the 02_model_comparison experiment
    _model_comp_results = Path(__file__).parent.parent / "02_model_comparison" / "results"
    run_dirs = sorted(_model_comp_results.iterdir()) if _model_comp_results.exists() else []
    if not run_dirs:
        raise RuntimeError(
...
def _rerank_with_gemini(moral_text: str, candidate_fables: list[str],
                        candidate_indices: list[int], gemini_client, gemini_model_name: str) -> list[int]:
    ...
    prompt = RERANK_PROMPT.format(moral=moral_text, fables_list=fables_list)
    try:
...
def run_approach_a(run_dir: Path, embed_run_dir: Path | None, top_k: int,
    ...
    gemini_model_name = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash-preview-05-20")
    ...
    if sample_size:
        _random.seed(42)
        moral_indices = _random.sample(moral_indices, min(sample_size, len(moral_indices)))
        print(f"  Sample mode: {sample_size} queries (random seed=42)")
    ...
    # Checkpoint: load any previously completed queries
    checkpoint_file = run_dir / "approach_a_checkpoint.json"
    reranked_results: dict[int, list[int]] = {}
    if checkpoint_file.exists():
        with open(checkpoint_file) as f:
            saved = json.load(f)
        reranked_results = {int(k): v for k, v in saved.items()}
        print(f"  Resuming from checkpoint: {len(reranked_results)} queries already done")

    # Gemini reranking
    for m_idx in tqdm(moral_indices, desc="  Gemini reranking"):
        if m_idx not in queries:
            continue
        if m_idx in reranked_results:
        ...
        reranked_results[m_idx] = _rerank_with_gemini(
            moral_texts[m_idx], candidate_texts, candidates, gemini_client, gemini_model_name
        )
        # Save checkpoint after every query
        with open(checkpoint_file, "w") as f:
            json.dump({str(k): v for k, v in reranked_results.items()}, f)
