...
def postprocess_summary(text: str, variant: str) -> str:
    ...
    # HuggingFace Transformers thinking format
    text = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL).strip()
    # MLX thinking format: thinking precedes <channel|>, answer follows
    if "<channel|>" in text:
        text = text.split("<channel|>")[-1].strip()
    lines = [ln.strip() for ln in text.split("\n") if ln.strip()]
    if not lines:
        return text
    return lines[0] if variant in ("direct_moral", "thinking_direct_moral") else lines[-1]
...
def generate_summary(model, tokenizer, fable_text: str, variant: str) -> str:
    ...
    enable_thinking = variant.startswith("thinking_")
    messages = [
    ...
    max_tokens = 1500 if enable_thinking else 150
