"""
run.py — Evaluate golden summaries for moral-to-fable retrieval.
...
Each config is tested with all 3 summary variants (direct_moral,
narrative_distillation, conceptual_abstract), plus a raw fable baseline.

Usage:
  python experiments/07_sota_summarization_oracle/run.py
  python experiments/07_sota_summarization_oracle/run.py --configs A B
  python experiments/07_sota_summarization_oracle/run.py --variants direct_moral
"""
...
CONFIGS = {
    ...
    "C": {
        ...
        "no_summary": True,  # doesn't use summary, just a prefix baseline
    },
}


def find_latest_summaries() -> Path:
    """Find the golden_summaries.json from the most recent generation run."""
...
def build_corpus_texts(
    ...
    """Build corpus texts for a given config + variant combination."""
    config = CONFIGS[config_key]
    template = config["template"]

    # Build a lookup from fable index to golden summary
    summary_lookup = {}
    for item in golden:
...
def run_experiment(config_keys: list[str], variants: list[str],
    ...
    """Run retrieval evaluation for specified configs and variants."""
    ...
    # Filter to requested variants that exist
    ...
    print(f"\n07_sota_summarization_oracle — Retrieval Evaluation")
    ...
    # Run each config x variant combination
    for config_key in config_keys:
        config = CONFIGS[config_key]

        if config.get("no_summary"):
            # Config C doesn't use summaries — run once
            print(f"\n  {'═' * 60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate golden summaries for moral-to-fable retrieval"
    )
    parser.add_argument(
        "--configs", nargs="+", choices=list(CONFIGS.keys()),
        default=list(CONFIGS.keys()),
        help="Which retrieval configs to run (default: all).",
    )
    parser.add_argument(
        "--variants", nargs="+",
        default=["direct_moral", "narrative_distillation", "conceptual_abstract"],
        help="Which summary variants to evaluate (default: all).",
    )
    parser.add_argument(
