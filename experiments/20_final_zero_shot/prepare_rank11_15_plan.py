...
def main() -> None:
    ...
    model_order = {model: idx for idx, model in enumerate(RANK11_15_MODELS)}
    ...
    write_csv(OUT_DIR / "gpu3_complete_rank11_15_missing.csv", fieldnames, rows)

    print(f"completed_run_ids={len(completed)}")
    print(f"gpu3_complete_rank11_15_missing={len(rows)}")
